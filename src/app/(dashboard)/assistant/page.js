...
const Assistant = () => {
  ...
  const startNewChat = () => {
    // Just reset state for a new chat, don't save to list yet
    const newChatId = uuidv4();
    const initialMessage = {
      role: "assistant",
      content: "Merhaba! Ben senin YKS koçunum. Sınav analizlerini yapabilirim. Bugün ne çalışmak istersin?",
  ...
  const deleteChat = (e, chatId) => {
    ...
    // Confirm deletion ? Maybe optional, but good for UX. Let's skip confirm for speed as user didn't ask, or add simple one.
    // User request was simple "button ekle". Let's just do it.
    ...
    if (chatExists) {
      updatedConversations = conversations.map((c) => {
        if (c.id === currentChatId) {
          // Generate title from first user message if it's "Yeni Sohbet"
          let title = c.title;
      ...
      // New chat being created upon first message
  ...
  return (
    <div className="flex h-full w-full relative">
      <title>YKS Asistanı - Puanİz</title>
      <meta name="description" content="YKS sürecinizde size rehberlik edecek yapay zeka destekli asistanınız." />

      {/* Sidebar - Desktop & Mobile */}
      <div
        className={`absolute md:relative inset-y-0 left-0 w-64 bg-slate-900 border-r border-white/10 z-30 transform transition-transform duration-300 ease-in-out md:translate-x-0 
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-1">
          {conversations.map((chat) => (
            ...
            >
              <MessageSquare
                size={16}
                className={`flex-shrink-0 ${currentChatId === chat.id ? "text-purple-400" : "text-slate-500 group-hover:text-purple-400"}`}
              />
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 bg-slate-950">
          ...
          <div className="flex flex-col w-full pb-4">
            {messages.map((msg, idx) => (
              ...
              >
                <div className="max-w-4xl mx-auto flex gap-4 md:gap-6">
                  {/* Avatar */}
                  <div
                    className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex-shrink-0 flex items-center justify-center overflow-hidden relative
            ...
            {loading && (
              <div className="w-full border-b border-white/5 px-4 py-6 md:px-8 bg-slate-900/50">
                <div className="max-w-4xl mx-auto flex gap-4 md:gap-6">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-emerald-600 flex-shrink-0 flex items-center justify-center overflow-hidden">
                    <div className="p-1.5 bg-white w-full h-full flex items-center justify-center relative">
                      <Image src="/favicon.png" alt="AI" fill className="object-contain" />
                    </div>
                  </div>
        ...
        {/* Input Area */}
        <div className="border-t border-white/10 bg-slate-900 p-4 md:p-6 flex-shrink-0">
          <form onSubmit={handleSend} className="max-w-4xl mx-auto relative">
            <div className="relative flex items-end gap-2 bg-slate-800 rounded-xl border border-white/10 p-2 focus-within:border-purple-500/50 focus-within:ring-1 focus-within:ring-purple-500/50 transition-all">
              <textarea
                value={input}
                ...
                className="w-full bg-transparent text-white pl-3 py-3 max-h-32 min-h-[50px] resize-none focus:outline-none placeholder:text-slate-500 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent"
                rows={1}
                style={{ height: "auto" }}
              />
              <button
                ...
                className="p-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed mb-0.5"
              >
                <Send size={18} />
              </button>
            </div>

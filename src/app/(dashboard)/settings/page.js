...
const ProfileSettings = () => {
  ...
  useEffect(() => {
    // Wait for auth init
    // Better to use onAuthStateChanged or our context.
    // auth.currentUser starts null.
    // Let's use listener here to be safe or just standard check if we assume ProtectedRoute handles it.
    // ProtectedRoute handles the wait. So auth.currentUser should be available or will be soon if we are here?
    // Actually, ProtectedRoute waits for loading=false.
    // So we can assume `auth.currentUser` is set if user is logged in.
    const unsubscribe = auth.onAuthStateChanged((currentUser) => {
      if (!currentUser) return; // ProtectedRoute handles redirect
      setUser(currentUser);
  ...
  return (
    <div className="space-y-6 pb-20">
      <title>Profil Ayarları - Puanİz</title>
      <meta name="description" content="Hesap ayarlarınızı ve tercihlerinizi güncelleyin." />
            ...
            <div className="relative group">
              <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-purple-500/30 group-hover:border-purple-500 transition-colors bg-slate-800 relative">
                <Image
                  ...
                  onError={(e) => {
                    // Next/Image onError is different but for simple fallback we might need a different approach or rely on the fact that avatarPreview has fallback logic above.
                    // However, Image component doesn't support direct onError to change src easily like img.
                    // But since line 92 already handles fallback: `formData.avatarUrl || ...`, we are mostly safe unless the URL itself is 404.
